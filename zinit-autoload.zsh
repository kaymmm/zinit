        ...
        if (( ! is_release )) {
            ( builtin cd -q "$local_dir" || return 1
              ...
              if (( ZINIT[annex-multi-flag:pull-active] >= 1 )) {
                  ...
                  ICE=()
                  (( ZINIT[annex-multi-flag:pull-active] >= 2 )) && command git pull --no-stat ${=ice[pullopts]:---ff-only} origin ${ice[ver]:-$main_branch} |& command grep -Ev '(FETCH_HEAD|up.to.date\.|From.*://)'
              }
        ...
        if [[ -d $local_dir/.git ]]; then
            (
                ...
                if (( OPTS[opt_-q,--quiet] )) {
                    ...
                    command git pull --recurse-submodules ${=ice[pullopts]:---ff-only} origin ${ice[ver]:-$main_branch} |& command grep -Ev '(FETCH_HEAD|up.to.date\.|From.*://)'
                }
            )
        fi
